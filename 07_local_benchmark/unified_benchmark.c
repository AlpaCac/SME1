...
int main(int argc, char **argv) {
  ...
  memref2d_f32 result = {0};
  for (int i = 0; i < 2; ++i) {
    memset(c, 0, (size_t)(m * n * sizeof(float)));
    result = gemm_step4_compute_stable(
        a, a, 0, m, k, k, 1,
        b, b, 0, k, n, n, 1,
        c, c, 0, m, n, n, 1);
  }
  ...
  double checksum_c = checksum_matrix(c, m, n);
  double checksum_desc = checksum_matrix(
      result.aligned + result.offset, result.sizes[0], result.sizes[1]);

  printf("elapsed_sec=%.9f\n", elapsed);
  printf("avg_ms=%.6f\n", elapsed * 1000.0 / (double)iters);
  printf("checksum_c=%.9f\n", checksum_c);
  printf("checksum_descriptor=%.9f\n", checksum_desc);
