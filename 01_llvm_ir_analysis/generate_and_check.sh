...
if [[ -n "${kernel_functions}" ]]; then
  ...
  while IFS= read -r function; do
    ...
    candidate_functions+=("${function} => ${demangled}")
...
if [[ "${#selected_functions[@]}" -eq 0 ]]; then
  ...
  if [[ "${#candidate_functions[@]}" -gt 0 ]]; then
    printf 'defined functions (IR symbol => demangled name):\n' >&2
    printf '  %s\n' "${candidate_functions[@]}" >&2
  fi
  printf 'set STENCIL_KERNEL_PATTERN or STENCIL_KERNEL_FUNCTIONS explicitly.\n' >&2
  exit 1
fi
